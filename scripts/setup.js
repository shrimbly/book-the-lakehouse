...
async function main() {
  ...
  const repoUrl = await askQuestion('Set your repository URL (NEXT_PUBLIC_REPO_URL)', envVars['NEXT_PUBLIC_REPO_URL'] || '');
  if (repoUrl) {
  ...
  // Write .env.local: preserve everything that was already there (esp. the
  // DATABASE_URL / BLOB_READ_WRITE_TOKEN / PG* keys pulled from Vercel) and
  // update or append the family-specific keys above. Order is grouped for
  // readability, then anything unknown is preserved at the bottom.
  const orderedSections = [
    {
      title: '# Database & Storage (from `vercel env pull`)',
      keys: ['DATABASE_URL', 'BLOB_READ_WRITE_TOKEN'],
    },
    {
      title: '# Access Rules',
      keys: ['FAMILY_PIN'],
    ...
    {
      title: '# Mary Mode',
      keys: ['MARY_IDS', 'COOKIE_PREFIX'],
    },
    {
      title: '# Stay Costs & Transfer details',
      keys: [
  ...
  for (const section of orderedSections) {
    const present = section.keys.filter((key) => envVars[key] !== undefined && envVars[key] !== '');
    if (present.length === 0) continue;
    finalEnvFileContent += `${section.title}\n`;
    for (const key of present) {
      finalEnvFileContent += `${key}=${envVars[key]}\n`;
      written.add(key);
    }
    finalEnvFileContent += '\n';
  }

  // Preserve any keys we didn't explicitly order (e.g. POSTGRES_URL, PGHOST,
  // VERCEL_OIDC_TOKEN, etc.) so `vercel env pull` data is never silently
  // dropped.
  const preserved = Object.keys(envVars)
    .filter((key) => !written.has(key))
    .filter((key) => envVars[key] !== undefined && envVars[key] !== '');
  if (preserved.length > 0) {
    finalEnvFileContent += `# Other (preserved from existing .env.local)\n`;
  ...
  if (databaseUrlExists) {
    ...
    if (doPush) {
      const pushSuccess = runCommand('npm run db:push');
      if (pushSuccess) {
  ...
  console.log(`  1. Commit any changes you've made (your secrets in .env.local are gitignored):`);
  console.log(`     ${color('git add -A && git commit -m "Configure family calendar"', C.gray)}`);
  ...
  console.log(`  Family-specific env vars (FAMILY_PIN, branding, payment details, etc.)`);
  console.log(`  can be set in the Vercel dashboard under Project → Settings → Environment Variables.\n`);
